...
stylesRequired = ['color', 'background-color', 'font-style', 'font-family', 'font-size', 'width', 'height', ['padding-top', 'padding-bottom', 'padding-left', 'padding-right'],
	['border-top', 'border-bottom', 'border-left', 'border-right'], 'border-color', 'border-radius', 'border-width', 'border-style', ['margin-top', 'margin-bottom', 'margin-left', 'margin-right']
]
...
module.exports = function(url, cb) {
	...
	for (displayType of displayTypes) {
		tests.push({
			...
			description: 'site has an element with a `display` of `' + displayType + '` assigned using an *internal* or *inline* style',
			assert: function(url, cb) {
				request(url, function(error, response, body) {
					if (!error && response.statusCode == 200) {
						jsdom.env(body, function(err, window) {
	...
	for (style of stylesRequired) {
		tests.push({
			...
			) + '` assigned using an *internal* or *inline* style',
			assert: function(url, cb) {
				request(url, function(error, response, body) {
					if (!error && response.statusCode == 200) {
						jsdom.env(body, function(err, window) {
