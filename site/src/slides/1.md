...
If you haven't turned in your enrollment agreement yet, do that! Email or print it out and give it to/email it to @alex
...
There's no due dates! You can complete assignments at your own convenience, and submit them whenever you want.
...
3. `cmd-opt-I` (or `ctrl-opt-I` if you're on a PC)
...
3. `cmd-shift-C` (or `ctrl-shift-C` if you're on a PC)
...
Let's take our code and deploy it out to the internet!
...
- Complete assignment `1` (type `submit 1` into @classbot and see what happens)
- Do the readings for session `2` (type `readings 2` into @classbot)
